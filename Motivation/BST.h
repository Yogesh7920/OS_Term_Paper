...
public:
    ...
    Node *uncle()
    {
        ...
        if (parent == nullptr or parent->parent == NULL)
            return NULL;
...
class RBTree
{
    ...
    static Node *BSTreplace(Node *x)
    {
        ...
        if (x->left != nullptr and x->right != NULL)
    ...
    void clear_rec(node parent)
    {
        if (parent == nullptr)
            return;
        clear_rec(parent->left);
        clear_rec(parent->right);
...
public:
    ...
    void random_fill(int n, int l)
    {
        random_device rd;
        mt19937 mt(rd());
        uniform_int_distribution<int> distribution(1, l + 1);
        for (int i = 0; i < n; i++)
        {
